...
export const TechTransition = ({
  ...
  const labelText = isToDiary
    ? 'Switching to Azhizen Diary Tech'
    : 'Switching to Azhizen Tech';
    ...
    function startAnimation() {
      ...
      function drawFrame() {
        ...
        if (!isFlipping) {
          ...
          if (progress <= 0) {
            if (onComplete) {
              setTimeout(onComplete, 400); // 400ms delay to let the user see the final state
            }
            return;
          }
        }
      ...
      <div className="mt-10 flex flex-col items-center gap-3">
        <div
          className={`w-6 h-6 border-2 ${
            isToDiary ? 'border-lime-500' : 'border-blue-500'
          } border-t-transparent rounded-full animate-spin`}
        />
        <p className="text-gray-400 font-medium tracking-widest text-[11px] uppercase font-[Poppins]">
          {labelText}
        </p>
      </div>
    </div>
